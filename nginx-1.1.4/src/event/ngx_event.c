...
ngx_shmtx_t           ngx_accept_mutex; //This is a share mutex!!Use to control the workers to accept request!!!
ngx_uint_t            ngx_use_accept_mutex;//mark which kind of accept strategy!!!
...
ngx_event_module_init(ngx_cycle_t *cycle)
{
    ...
    shm.size = size;     /* Set the share memory size*/
    shm.name.len = sizeof("nginx_shared_zone"); /* caculate the string lengths*/
    shm.name.data = (u_char *) "nginx_shared_zone";
    shm.log = cycle->log;

    if (ngx_shm_alloc(&shm) != NGX_OK) {//Here,we alloc the share memory!Use the mmap!!
        return NGX_ERROR;
    }

    shared = shm.addr;//Ther shared address!!

    ngx_accept_mutex_ptr = (ngx_atomic_t *) shared;//This is the ngx_accept_mutex address!!
    ngx_accept_mutex.spin = (ngx_uint_t) -1;
    
    //Here,we create the ngx_accept_mutex!!!
...
ngx_event_use(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (ecf->use != NGX_CONF_UNSET_UINT) {//Test whether we have set the event scheme!!!
        return "is duplicate";
    }
    ...
    for (m = 0; ngx_modules[m]; m++) {//traverse the event_modules to find the event scheme pointed by use command!!!
        if (ngx_modules[m]->type != NGX_EVENT_MODULE) {
            continue;
        }
        ...
        if (module->name->len == value[1].len) {
            if (ngx_strcmp(module->name->data, value[1].data) == 0) {
                ecf->use = ngx_modules[m]->ctx_index; //Here,we mark which event_module we will used!!!
