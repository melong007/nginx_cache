...
ngx_event_pipe(ngx_event_pipe_t *p, ngx_int_t do_write)
{
    ...
    for ( ;; ) {
        if (do_write) {
            p->log->action = "sending to client";//really send data to client!!!
        ...
        if (ngx_event_pipe_read_upstream(p) == NGX_ABORT) {//read from the upstream!!
            return NGX_ABORT;
        }
...
ngx_event_pipe_read_upstream(ngx_event_pipe_t *p)
{
    ...
    for ( ;; ) {
        ...
        if (p->preread_bufs) {
            ...
            n = p->upstream->recv_chain(p->upstream, chain);//recv data from the upstream!!!
