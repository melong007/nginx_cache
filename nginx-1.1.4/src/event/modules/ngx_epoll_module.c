...
struct epoll_event {
    uint32_t      events;//use to stat which kind of event should be listen!!
    epoll_data_t  data;
};
...
ngx_epoll_init(ngx_cycle_t *cycle, ngx_msec_t timer)
{
    ...
    nevents = epcf->events;//how much event amount we can monitor!!!
    ngx_io = ngx_os_io;    //Here,Set the really socket data read write handler!!!
    ngx_event_actions = ngx_epoll_module_ctx.actions;//important! Set the Golobal ngx_event_actions as epoll actions!!!
...
ngx_epoll_add_connection(ngx_connection_t *c)
{
    ...
    if (epoll_ctl(ep, EPOLL_CTL_ADD, c->fd, &ee) == -1) {//add the connection to epoll pool!!!
