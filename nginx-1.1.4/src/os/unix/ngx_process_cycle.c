...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
	...
	//Here,we start create worker and manager processes!!!!
...
//Here,the worker will continue to excute ngx_worker_processor_cycle
...
ngx_start_cache_manager_processes(ngx_cycle_t *cycle, ngx_uint_t respawn)
{
	...
	//Here,create cache manager process!!!
	...
	//Here,create cache loader process!!!
    ngx_spawn_process(cycle, ngx_cache_manager_process_cycle,
...
//Here,very important!!we will initilize some important config parameter,like 
//ngx_event_core_module.ngx_event_process_init(),this function very important for the event scheme!!!
    ...
    for ( ;; ) {
        ...
        ngx_process_events_and_timers(cycle);//We start to process events!!!
...
//-------------------->Here,we should call all the module's  init_process()
//In fact,only the event module has the init_process
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->init_process) {
            if (ngx_modules[i]->init_process(cycle) == NGX_ERROR) {
                /* fatal */
