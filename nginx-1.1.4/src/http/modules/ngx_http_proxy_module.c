...
typedef struct {
    ngx_str_t                      key_start; //Here,set the cache_key!!
    ngx_str_t                      schema;    //Here,we set the upstream scheme;
...
typedef struct {
    ngx_http_status_t              status; //the http status line!!!
    ngx_http_proxy_vars_t          vars;   			 
...
static ngx_command_t  ngx_http_proxy_commands[] = {
    ...
    { ngx_string("proxy_pass"), //only the "proxy_pass" directive should be configed in loc_conf!!!
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    ngx_http_proxy_loc_conf_t  *plcf;
    //Here,we just alloc ngx_http_upstream_t{},
    if (ngx_http_upstream_create(r) != NGX_OK) { //just alloc a upstream struct!!!
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }

    ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_proxy_ctx_t)); //alloc a proxy_ctx,
    if (ctx == NULL) {
        return NGX_ERROR;
    }

    ngx_http_set_ctx(r, ctx, ngx_http_proxy_module);  	        //proxy_module desposit ctx!!!
    plcf = ngx_http_get_module_loc_conf(r, ngx_http_proxy_module);//ngx_http_proxy_loc_conf_t

    u = r->upstream;
    /*If the plcf->proxy_lengths == NULL,Means the proxy_pass command style is as fellow!*/
    /*##format: --------->    proxy_pass http://www.sinaxx.com;*************/ 

    /*## In this style,plcf->upstream.upstream backend server has been related!
    ...
    */
    ...
    //If the plcf->proxy_lengths != NULL,Means the proxy_pass command style is as fellow!
    /*##format: --------->    proxy_pass blog.$host;*************/ 

    /*##In this style,plcf->upstream.upstream hasn't related!!
        In the ngx_http_proxy_eval() will alloc a ngx_http_upstream_resolved_t struct will be use in after process
         u->resolved = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_resolved_t));
        In the ngx_http_upstream_init_request(){

             if (u->resolved == NULL) {
              //Means,the plcf->upstream.upstream is realted,state this is the previous style!
                 uscf = u->conf->upstream;
             } else {
              //Here,Means we need to do the host reslove!

             } 
        }
    */
        if(ngx_http_proxy_eval(r, ctx, plcf)!= NGX_OK) { //here,we will alloc a ngx_http_upstream_resolved_t struct
            return NGX_HTTP_INTERNAL_SERVER_ERROR;
        }
    }

    //Means,at present we must get upstream server(for haven't variable proxy_pass) or backend_ip()
    u->output.tag = (ngx_buf_tag_t) &ngx_http_proxy_module;
    //Means,this request will use which upstream_conf
    u->conf = &plcf->upstream; 			//Here,means the 
    ...
    u->pipe->input_filter = ngx_http_proxy_copy_filter; //watch out!!
    u->pipe->input_ctx = r;
    ...
    ngx_http_proxy_loc_conf_t *plcf)
{
    ...
    ngx_url_t             url;
    ngx_http_upstream_t  *u;
    //Here,Get the really proxy string!!!
    ...
    } else { //Here,state the proxy string must started with "http://" or "https://"
    ...
    url.uri_part = 1;
    url.no_resolve = 1; //!!!!!!! Here,Means Don't do the url resolve!!!
    //now parse the url to get the really hostname and port!!
    ...
    //Now,we need to alloc a resolve struct!
    u->resolved = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_resolved_t));
    if (u->resolved == NULL) {
        return NGX_ERROR;
    }
...
ngx_http_proxy_create_key(ngx_http_request_t *r)
{
    ...
    if (plcf->cache_key.value.len) { //If set the cache key,we get the cache key!!!

        if (ngx_http_complex_value(r, &plcf->cache_key, key) != NGX_OK) { //Means,Get the value 
            return NGX_ERROR;
        }
...
ngx_http_proxy_create_request(ngx_http_request_t *r)
{
    ...
    *b->last++ = CR; *b->last++ = LF;
    ...
    if (plcf->body_set == NULL && plcf->upstream.pass_request_body) {
        ...
        body = u->request_bufs;
        u->request_bufs = cl; //Here,we will insert the request header before the body data! 
...
static ngx_int_t
ngx_http_proxy_add_variables(ngx_conf_t *cf)//add all the proxy variables!!!
{
...
//recite it,proxy_pass will will really call the upstream module by the ngx_http_proxy_handler!!!
static char *
ngx_http_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    //only one proxy_pass command  can be layed in a location!
    //Means one location only have one related proxy_pass command!
    ...
    clcf->handler = ngx_http_proxy_handler;
    //Here,we set the handler,when we call the ngx_http_core_find_config_phase,we will
    //set the this handler to r->content_handler! and the other content phase handler will not be used!!!
   ...
   /*##format: --------->    proxy_pass http://blog.$host;*************/ 
    n = ngx_http_script_variables_count(url);//Means how much variables used in this parameter!!!
    ...
    if (n) {
        ...
        sc.cf = cf;
        sc.source = url;		    //Here,The parameter include variable's we need to parse!!!
        sc.lengths = &plcf->proxy_lengths;  //Here,proxy_lengths
        sc.values = &plcf->proxy_values;
        sc.variables = n;		        //has n's variables!
        sc.complete_lengths = 1;
        sc.complete_values = 1;
    ...
    u.url.data = url->data + add;  //the host
    u.default_port = port;         //the port
    u.uri_part = 1;
    u.no_resolve = 1; //At Here,Don't resolve the host!!!
    
    //Here,we parse a upstream!!
    plcf->upstream.upstream = ngx_http_upstream_add(cf, &u, 0);
    ...
    //Here,we will set the url result the plcf->vars!
    //Means,If we use proxy_pass http://cc_server/cityinfo.json$cache_args!!!
    //The really uri is "/cityinfo.json$sgti_args"
...
ngx_http_proxy_redirect(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (plcf->redirects == NULL) {
        plcf->redirects = ngx_array_create(cf->pool, 1,sizeof(ngx_http_proxy_redirect_t));
        if (plcf->redirects == NULL) {
            return NGX_CONF_ERROR;
        }
    }
...
ngx_http_proxy_set_vars(ngx_url_t *u, ngx_http_proxy_vars_t *v)
{
    ...
    /*Here,we set the u->uri to the plcf->vars.uri*/
    v->uri = u->uri;
}
