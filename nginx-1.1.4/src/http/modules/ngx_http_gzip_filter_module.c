...
ngx_http_gzip_filter_buffer(ngx_http_gzip_ctx_t *ctx, ngx_chain_t *in)
{
    ...
    buffered = 0; /**Means,how much we has buffered**/
    ll = &ctx->in;/**Here,the ctx->in is the local buffered_list!! **/
    ...
    while (in) {
        ...
        size = b->last - b->pos;
        buffered += size;  //caculate how much buffered!!

        if (b->flush || b->last_buf || buffered > conf->postpone_gzipping) {
            ctx->buffering = 0; //under this condition,we no need to postpone and buffering the data!!
        }

        if (ctx->buffering && size) {
            /**Now,we need to alloc a new buff to buffering the in_data!!!**/
            buf = ngx_create_temp_buf(r->pool, size);
            if (buf == NULL) {
                return NGX_ERROR;
            }
            /**Now,we Copy the in_data!!!***/
            buf->last = ngx_cpymem(buf->pos, b->pos, size); /**Copy data from b->pos to buf->pos**/
            b->pos = b->last;
            /****/
