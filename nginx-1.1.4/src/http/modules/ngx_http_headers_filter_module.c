...
struct ngx_http_header_val_s {
    ngx_http_complex_value_t   value;		//this is a value
    ngx_uint_t                 hash;		
    ngx_str_t                  key;			//this is the http_header name
    ngx_http_set_header_pt     handler;
    ngx_uint_t                 offset;
};
...
ngx_http_add_header(ngx_http_request_t *r, ngx_http_header_val_t *hv, ngx_str_t *value)
{
    ...
    if (value->len) {
        h = ngx_list_push(&r->headers_out.headers);//Here,we will add the header to the client!
        if (h == NULL) {
            return NGX_ERROR;
        }

        h->hash = hv->hash;	//This is header's hash
        h->key = hv->key;	//header's name
        h->value = *value;	//Header's value!!
    }
...
ngx_http_headers_add(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    hv->hash = 1;
    hv->key = value[1];			//this is the header's name!!!
    hv->handler = ngx_http_add_header;	//this is the add handler!
    hv->offset = 0;
