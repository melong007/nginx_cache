...
ngx_http_limit_req_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "zone=", 5) == 0) {
            ...
            name.data = value[i].data + 5;	/*Get the name start position*/

            p = (u_char *) ngx_strchr(name.data, ':');

            if (p) {
                *p = '\0';
                name.len = p - name.data;	/*Get the name length*/

                p++;	//

                s.len = value[i].data + value[i].len - p; /*Get the size string length*/
                s.data = p;					/*Get the size start address*/

                size = ngx_parse_size(&s);	/*translate the size string to size*/
                if (size > 8191) {
                    continue;
                }
            }
        ...
        if (ngx_strncmp(value[i].data, "rate=", 5) == 0) {
            ...
            len = value[i].len;
            p = value[i].data + len - 3;/*point to the subfix "r/s" or "r/m"*/
            ...
            rate = ngx_atoi(value[i].data + 5, len - 5);	/*Get the rate*/
            if (rate <= NGX_ERROR) {
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
...
{
    ngx_http_limit_req_conf_t  *lrcf = conf;  //get this location's ngx_http_limit_req_conf_t;
...
ngx_http_limit_req_init(ngx_conf_t *cf)
{
    ...
    *h = ngx_http_limit_req_handler;	//register the process handler!!!
