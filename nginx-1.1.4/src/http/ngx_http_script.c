...
ngx_http_complex_value(ngx_http_request_t *r, ngx_http_complex_value_t *val,ngx_str_t *value)
{
    ...
    while (*(uintptr_t *) e.ip) { //Here,we caculate the value length!
        lcode = *(ngx_http_script_len_code_pt *) e.ip;
        len += lcode(&e);
    }

    value->len = len;
    value->data = ngx_pnalloc(r->pool, len); //Here,we alloc the value stored space!!
    if (value->data == NULL) {
        return NGX_ERROR;
    }
    ...
    while (*(uintptr_t *) e.ip) {
        ...
        code((ngx_http_script_engine_t *) &e);  //call the code handler to get the complex variable related value!
    }
...
ngx_http_compile_complex_value(ngx_http_compile_complex_value_t *ccv)
{
    ...
    v = ccv->value;	//Here,Get the value!!!
    ...
    for (i = 0; i < v->len; i++) {	//
        if (v->data[i] == '$') {	//Means this is a variable 
            if (v->data[i + 1] >= '1' && v->data[i + 1] <= '9') {
                nc++;	//use the $1 $2 state a 

            } else {	//caculate the variable count!
                nv++;
            }
        }
    }
...
ngx_http_script_compile(ngx_http_script_compile_t *sc)
{
	...
	//sc->source is a string! scan the how string!!!
    for (i = 0; i < sc->source->len; /* void */ ) {

        name.len = 0;

        if (sc->source->data[i] == '$') {//get a variable!!
...
{
    if (*codes == NULL) {	//If is Null,we need to create a array!
        *codes = ngx_array_create(pool, 256, 1);	//Means has 256 bytes!!
        if (*codes == NULL) {
            return NULL;
        }
    }

    return ngx_array_push_n(*codes, size);	//now,we need to append a element!!
}
...
ngx_http_script_equal_code(ngx_http_script_engine_t *e)
{
    ...
    e->sp--;	//
    val = e->sp;	//this is the va
    res = e->sp - 1;

    e->ip += sizeof(uintptr_t);//Means,move to the next!!
...
{
    ngx_http_script_value_code_t  *code;	//Here,we point to the top position!

    code = (ngx_http_script_value_code_t *) e->ip; //Here,we Point value code function!
...
ngx_http_script_var_code(ngx_http_script_engine_t *e)
{
    ...
    e->ip += sizeof(ngx_http_script_var_code_t);
	//Here,we need to get the really variable related value!
	//code->index is set by the ngx_http_rewrite_variable()
    value = ngx_http_get_flushed_variable(e->request, code->index);
    ...
    if (value && !value->not_found) {
        ...
        *e->sp = *value;	//Means we get the value!
        e->sp++;			//Means we get a variable related value;
