...
ngx_http_add_variable(ngx_conf_t *cf, ngx_str_t *name, ngx_uint_t flags)
{
    ...
    //Here,traversal all the variable!!!
    for (i = 0; i < cmcf->variables_keys->keys.nelts; i++) {
        ...
        v = key[i].value;//Means there is a same variable in the array!!

        if (!(v->flags & NGX_HTTP_VAR_CHANGEABLE)) {	//Means a 
    ...
    //Here,Web create  a ngx_http_variable_t,Means  we has new a variable
    ...
    ngx_strlow(v->name.data, name->data, name->len);//change to lower format!!!
    ...
    //add it into the variables_keys!!!
...
ngx_http_get_variable_index(ngx_conf_t *cf, ngx_str_t *name)
{
    ...
    //cmcf->variables.elts store all the variable elements!!
    ...
    if (v == NULL) {
        ...
        for (i = 0; i < cmcf->variables.nelts; i++) {//find the variable!!!
    ...
    v->data = 0;
    v->flags = 0;
    v->index = cmcf->variables.nelts - 1;//This variable's index.
 ...
 *struct ngx_http_variable_s {
    ...
    ngx_http_set_variable_pt      set_handler;	//very important!
...
*/
	...
	//Here,we need to get the variable!
	//Means,every variable has it's own handler(get_handler,set_handler)
...
ngx_http_regex_compile(ngx_conf_t *cf, ngx_regex_compile_t *rc)
{
    ...
    re->regex = rc->regex;
    re->ncaptures = rc->captures; //How much captured

    cmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_core_module);
    cmcf->ncaptures = ngx_max(cmcf->ncaptures, re->ncaptures);

    n = (ngx_uint_t) rc->named_captures; //For the named_captures;
...
ngx_http_variables_add_core_vars(ngx_conf_t *cf)
{
	...
	//Here,we add the build-in variables into the cmcf->variables_keys!!!
	...
	{
		//Means the build-in variables is READONLY_KEY
...
ngx_http_variables_init_vars(ngx_conf_t *cf)
{
    ...
    hash.key = ngx_hash_key;   //Here,
    hash.max_size = cmcf->variables_hash_max_size;
    hash.bucket_size = cmcf->variables_hash_bucket_size;
    hash.name = "variables_hash";
    hash.pool = cf->pool;
    hash.temp_pool = NULL;
    //very import! we initilize the variables_hash!!
    if (ngx_hash_init(&hash, cmcf->variables_keys->keys.elts, cmcf->variables_keys->keys.nelts)
        != NGX_OK)
    {
        return NGX_ERROR;
    }
