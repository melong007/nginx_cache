...
{
    ngx_event_t         *rev;
    ngx_http_log_ctx_t  *ctx;//request desposit ctx!!!
    ...
    rev = c->read;  //Means the read event!!!
    rev->handler = ngx_http_init_request;//Here,set this connection handler as read http_request!!!
    ...
    if (ngx_handle_read_event(rev, 0) != NGX_OK) {//add to the epoll!!!!!
#if (NGX_STAT_STUB)
        (void) ngx_atomic_fetch_add(ngx_stat_reading, -1);
#endif
        ngx_http_close_connection(c);
...
ngx_http_init_request(ngx_event_t *rev)
{
    ...
    hc = c->data;	//Use to contain all the http request in this http connection!!!

    if (hc == NULL) {	//first time this data must be NULL!!!
    ...
    if (r) {
        ...
        r = ngx_pcalloc(c->pool, sizeof(ngx_http_request_t));//Here,alloc a http_request!!!
        if (r == NULL) {
            ngx_http_close_connection(c);
            return;
        }
    ...
    c->data = r;	//Means belong to this request?????
...
/* find the server configuration for the address:port */
    ...
    r->connection = c;//Means this request belong to this connection!!
    if (port->naddrs > 1) {
    ...
    r->loc_conf = cscf->ctx->loc_conf;		//Here,get the pointed server's all locaiont
											//Here,update the read_event handler!!!
    rev->handler = ngx_http_process_request_line;//set the rev->handler to ngx_http_process_request_line to parse request!!!
    ...
    if (c->buffer == NULL) {
//#################Here,we create the header's buffer!!!
...
//################Use to store the temp data!!!
    ...
    if (ngx_list_init(&r->headers_out.headers, r->pool, 20,sizeof(ngx_table_elt_t))
        != NGX_OK)
    {
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
		...
		//Here,we parse the http request_line!!!!!
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            ...
            if (r->header_in->pos == r->header_in->end) {//Means,we have used the header buffer!!!

                rv = ngx_http_alloc_large_header_buffer(r, 0);//alloc a header buffer!!
            ...
            n = ngx_http_read_request_header(r); //read a header line!!!
        ...
        rc = ngx_http_parse_header_line(r, r->header_in,cscf->underscores_in_headers); //start to parse a header line!!!
...
//Means all the http header are stored in the headers_in.headers!!!!
//add this headers to the headers_in.headers list!!!
            ...
            h->hash = r->header_hash; //caculate the hash value!!
...
//---------------------------------------------->>important!!---------------------------------------!
//If this header is formaly used http_header,we save to the headers_in
			//ngx_table_elt_t   *h;
            if (hh && hh->handler(r, h, hh->offset) != NGX_OK) {  //here,call the related header parser defined in  ngx_http_headers_in[]  
                return;
            }
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (rev->ready) {	//Here,we read n bytes from the fd to the header_in buffer!
    ...
    r->header_in->last += n;	//increase the header buffer!!!
...
{
    if (ngx_http_find_virtual_server(r, r->headers_in.server.data,r->headers_in.server.len)
        == NGX_ERROR)
    {
...
ngx_http_run_posted_requests(ngx_connection_t *c)
{
    ...
    for ( ;; ) {
        ...
        r = c->data;
        pr = r->main->posted_requests; //Get a posted_request!

        if (pr == NULL) {
            return;
        }

        r->main->posted_requests = pr->next; //Here,point to the next posted_request!

        r = pr->request;    //The subrequest!
        ...
        r->write_event_handler(r); //call the handler!!
    }
}
...
ngx_http_free_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (r->connection->timedout) {
        ...
        if (clcf->reset_timedout_connection) { //Means,reset the connection!!!
            linger.l_onoff = 1;
            linger.l_linger = 0;
