...
ngx_http_upstream_create(ngx_http_request_t *r)
{
    ...
    u = ngx_pcalloc(r->pool, sizeof(ngx_http_upstream_t));//Here,we alloc a new upstream!!!
    if (u == NULL) {
        return NGX_ERROR;
    }
...
ngx_http_upstream_init(ngx_http_request_t *r)
{
    ...
    if (c->read->timer_set) {//Means we had finishded read the client request
        ngx_del_timer(c->read);
    }
    if (ngx_event_flags & NGX_USE_CLEAR_EVENT) {
        if (!c->write->active) {//Here,Means add client connection to the epool write!!!
    ...
    //start to prepare the request which sended to upstream server!!
}

//-------------------------->Here,very important,NGX_HTTP_CACHE default is 1,so the object default will be cached!!!
static void
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
...
#if (NGX_HTTP_CACHE)
    if (u->conf->cache) {//First we Check Doesn't we have a locale cached object!!
        ngx_int_t  rc;
    ...
    if (r->request_body) {
        u->request_bufs = r->request_body->bufs;//Use to received request_body!!!
    }

    /*
    ##################Here,we create the client request!
    u->create_request = ngx_http_proxy_create_request; //create the request which will be send to original server!!!
    */

    if (u->create_request(r) != NGX_OK) {//Now we initilize a http_request which will be send to origin server 
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
        return;
    }
    ...
    //Here,set the upstream write_handler!!
    u->output.alignment = clcf->directio_alignment;
    u->output.pool = r->pool;
    u->output.bufs.num = 1;
    u->output.bufs.size = clcf->client_body_buffer_size;
    //Here,this write_filter just used in the ngx_http_upstream_send_request()
    u->output.output_filter = ngx_chain_writer;	//watch out! The write handler!!!
    ...
    //u->state will save the upstream current state!
    //for example,how much we have read from upstream server!!
    ...
    if (u->resolved == NULL) {
       //Means we use the default upstream!!!
        ...
        ctx = ngx_resolve_start(clcf->resolver, &temp);//do the resovle operation!!!
        if (ctx == NULL) {
            ngx_http_upstream_finalize_request(r, u,
                                               NGX_HTTP_INTERNAL_SERVER_ERROR);
            return;
...
found:
    //########################  very important!!!
    //Here,we will call the pee.init() to get a really backend ip()
    if (uscf->peer.init(r, uscf) != NGX_OK) { //
/*
    //Here,set the get handler,call in ngx_http_upstream_connect
...
*/
    ...
    ngx_http_upstream_connect(r, u);//start to connect!!!
}
...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (c == NULL) {
        ...
        c->file_cache = u->conf->cache->data; /**Means,belong to which cache_path**/
...
ngx_http_upstream_handler(ngx_event_t *ev)
{
    ...
    c = ev->data; //this is the upstream connection!
    r = c->data;  //This is the origin request!
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    //#################  Very important!
    //Here,,we do the really connect operation!
    rc = ngx_event_connect_peer(&u->peer);//Here,we create a really connection!!!
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
    ...
    c->data = r;
/*
   ngx_event_connect_peer() will set the data recv  and send handler!!
    c->recv = ngx_recv;
    c->send = ngx_send;
    c->recv_chain = ngx_recv_chain;
    c->send_chain = ngx_send_chain;
 */
    //This is the upstream connection event desposit handler!
    //c->write is ngx_event_s{  ngx_event_handler_pt *handler;}
    //c->read  is ngx_event_s{  ngx_event_handler_pt *handler;}

//##############################  really important!
    c->write->handler = ngx_http_upstream_handler;//Here,Set the event process handler
    c->read->handler = ngx_http_upstream_handler;
/* ngx_http_upstream_handler() Just call the u->write_event_handler or u->read_event_handler!!
    if (ev->write) {
        u->write_event_handler(r, u);
    } else {
        u->read_event_handler(r, u);
...
*/
    //This is the upstream request disposit handler!!
    u->write_event_handler = ngx_http_upstream_send_request_handler;
    u->read_event_handler = ngx_http_upstream_process_header;
    ...
    if (u->ssl && c->ssl == NULL) {//Before the send request send,we do the ssl initilize!
...
ngx_http_upstream_send_request(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    /*recite the function,ngx_output_chain() is a send data handler!!!
    u->output.output_filter = ngx_chain_writer;	//watch out! The write handler!!!
    u->output.filter_ctx = &u->writer;
    */
    rc = ngx_output_chain(&u->output, u->request_sent ? NULL : u->request_bufs);//Send the request data!!!
    u->request_sent = 1;
    if (rc == NGX_ERROR) {
        ngx_http_upstream_next(r, u, NGX_HTTP_UPSTREAM_FT_ERROR);
        return;
    ...
    if (c->write->timer_set) {//http header has been send,in this connection we just need to read data!!
    ...
    if (c->read->ready) {
         ...
         */
        ngx_http_upstream_process_header(r, u);//start to process the response header!!!
        return;
    }
#endif
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    for ( ;; ) {
        //####################   recv data from the upstream server!!!
        ...
        u->buffer.last += n;//update how much we have recv!!!
#if 0
        u->valid_header_in = 0;
        u->peer.cached = 0;
#endif
       ...
       */
        rc = u->process_header(r);//call the process handler!!!
        if (rc == NGX_AGAIN) {
    ...
    /* rc == NGX_OK ,Means successfuly processed the header!!!*/
    ...
    //Here,we will copy the upstream_request to client_request->headers_out
    if (ngx_http_upstream_process_headers(r, u) != NGX_OK) {//the upstream despoit the left work!!!
        return;
    }
    //#############################  very important!!!!!!!
    //all the main request will goto send response,but the subrequest just to copy data to main request!
    if (!r->subrequest_in_memory) {//the first request,subrequest_in_memory must not been set!!!
        ngx_http_upstream_send_response(r, u);//start to send the response!!!
        return;
    }
...
ngx_http_upstream_process_headers(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    part = &u->headers_in.headers.part;
    h = part->elts;
    //Here,we copy the upstream_request header into the client_request headers_out
    for (i = 0; /* void */; i++) {
        if (i >= part->nelts) {
            if (part->next == NULL) {
                break;
    ...
    ngx_http_upstream_t *u)
{
    ...
    for ( ;; ) {
        ...
        n = c->recv(c, b->last, size);//recv data from origin!!!
        ...
        u->state->response_length += n;//update the response length
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    ngx_connection_t          *c;
    ngx_http_core_loc_conf_t  *clcf;
    //Here,we send upstream respones header to client!!!
    ...
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);
    //##################  important!!
    //Means,that set the proxy_buffering off and send the response 
    if (!u->buffering) {
        if (u->input_filter == NULL) {
            u->input_filter_init = ngx_http_upstream_non_buffered_filter_init;
            u->input_filter = ngx_http_upstream_non_buffered_filter;
        ...
        n = u->buffer.last - u->buffer.pos;//Means,how much data we have get from the original server!!!
    ...
    //############# Means data Doesn't send to client directly!!!!
    ...
    //###############  Here,will do the no_cache check!!!
    ...
    if (u->cacheable) {//If we should cache the object!!!
        time_t  now, valid;
...
#endif
    //######################## Very important,Set the read pipe handler!!
    p = u->pipe;
    ...
    if (u->cacheable) {//alloc the cache file buffer!!!
    ...
    u->read_event_handler = ngx_http_upstream_process_upstream;	   //process the upstream response!!!
    r->write_event_handler = ngx_http_upstream_process_downstream; //send the upstream response to client!!!
    ...
    if (u->peer.connection) {
        ...
        if (u->store) {
            ...
            if (p->upstream_eof || p->upstream_done) {
                ...
                {
                    ngx_http_upstream_store(r, u);//store the object!!!
...
//compile a upstream!! include server real server!!!
{
    char                          *rv;
    void                          *mconf;
    ngx_str_t                     *value;
    ...
    value = cf->args->elts;
    u.host = value[1];//for example,  upstream backend_cc_server { server xxxx; server xxxx2;  }
    u.no_resolve = 1; //At here,we don't resolve,
    //here,Create the ngx_http_upstream_srv_conf_s, It is stored all the upstream server config in the umcf!!!
...
//Here,use to complex a upstream_server!!!
static char *
ngx_http_upstream_server(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_uint_t                   i;
    ngx_http_upstream_server_t  *us; //state one backend server!!
...
ngx_http_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
    ...
    uscfp = umcf->upstreams.elts;
    //Means that all the upstream directive pointed configure should be stored in the umcf!!!
    //ngx_http_upstream_main_conf_t  *umcf;
     ...
     * typedef struct {
       ...
       ngx_array_t        upstreams;//ngx_http_upstream_srv_conf_t,all the upstream directive config stored in here
    ...
    */
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        return uscfp[i];//Means,have a same upstream in the conf
    }
    //Means push a new upstream_cluster!!
