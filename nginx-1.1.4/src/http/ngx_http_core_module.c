...
static ngx_command_t  ngx_http_core_commands[] = {
    ...
    { ngx_string("client_header_timeout"),
      ...
      NGX_HTTP_SRV_CONF_OFFSET,//Where
    ...
    { ngx_string("location"),
      ...
      NGX_HTTP_SRV_CONF_OFFSET,//point to which conf!!!
      0,
      NULL },

    { ngx_string("listen"),   //Here,listen only used in the SRV_CONF parse!!! 
...
ngx_module_t  ngx_http_core_module = {
    NGX_MODULE_V1,
    &ngx_http_core_module_ctx,             /* module context,Here,
											  very important for the ngx to 
											  parse the Request!!!!*/
...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    while (ph[r->phase_handler].checker) {
		...
		//If one phase_handler.checker return NGX_OK,the continue phase_handler 
		//will be omited and directlly return!!!
        if (rc == NGX_OK) {
            return;
        }
    }
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    if (rc == NGX_OK) {//Here,If the ph->handler return NGX_OK,ngx_http_core_generic_phase should change it to NGX_AGAIN,Because
        r->phase_handler = ph->next;//Here,we 
        return NGX_AGAIN;
    }
...
ngx_http_core_find_config_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    rc = ngx_http_core_find_location(r);	//Here,we set a suitable location!
	...
	//Here,get the location which match the request! 
	...
	ngx_http_update_location_config(r);//critical important!!!
	/*
    if (clcf->handler) {	
...
/*Here,ph->handler!!!*/
    ...
    if (clcf->satisfy == NGX_HTTP_SATISFY_ALL) {
		/*Here,Means throught the checker!!!*/
        if (rc == NGX_OK) {
            r->phase_handler++;	//we need to check the next access!
            return NGX_AGAIN;
        }
...
ngx_http_core_content_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
	...
	//remember,If r->content_handler isn't NULL,
	//the cmcf->phase_engine.handler's content_phase will not be excuted!!!!
...
//rc = ph[r->phase_handler].checker(r, &ph[r->phase_handler]);
    ph++;	//Here,we move to the next handler!!!

    if (ph->checker) {
		//Does still have other content_handler??
        r->phase_handler++;
        return NGX_AGAIN;
    }
...
ngx_http_core_find_location(ngx_http_request_t *r)
{
    ...
    //Here,get the top level loc_conf;
...
ngx_http_core_find_static_location(ngx_http_request_t *r, ngx_http_location_tree_node_t *node)
{
    ...
    len = r->uri.len;//Here,uri Means the request's path
    uri = r->uri.data;
    ...
    for ( ;; ) {
        ...
        if (len > (size_t) node->len) {
            ...
            if (node->inclusive) {
                ...
                r->loc_conf = node->inclusive->loc_conf;//save the inclusive loc_conf!!!
                rv = NGX_AGAIN;

                node = node->tree;//we need to search the the node's tree,
                uri += n;
                len -= n;
        ...
        if (len == (size_t) node->len) {
            if (node->exact) {
                r->loc_conf = node->exact->loc_conf;//Here get the location!!!
                return NGX_OK;
...
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    ngx_http_core_main_conf_t   *cmcf;
//recite ,ngx_http_core_server() create the new ctx
//this ctx will save to the top http_ctx.main_conf[ngx_http_core_modulel->ctx_index].server
//All the server created ctx will be save to here!!!
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }

    http_ctx = cf->ctx;//Here,Get the http_ctx!!!!

    ctx->main_conf = http_ctx->main_conf;//Here main conf keep the same as http_ctx!!!

    /* the server{}'s srv_conf */
    ctx->srv_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);//create the new  srv_conf[]
    if (ctx->srv_conf == NULL) {
        return NGX_CONF_ERROR;
    }

    /* the server{}'s loc_conf */
    ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);//create the new loc_conf[]
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        if (module->create_srv_conf) {
            ...
            ctx->srv_conf[ngx_modules[i]->ctx_index] = mconf;//save the mconf to the ctx->srv_conf!!!!
        }

        if (module->create_loc_conf) {//create all the http_module's private conf and store in the loc_conf[]
...
//---------------------->Here,server {} is belong to ngx_http_core_module,
//---------------------->So first get the ngx_http_core_srv_conf_t from the new ctx->srv_conf!!!

    cscf = ctx->srv_conf[ngx_http_core_module.ctx_index];//cscf is current ctx's ngx_http_core_srv_conf_t!!!
	cscf->ctx = ctx; 									 //Means mark this core_srv_conf_t belong to which ctx!

//---------------------->Here,very important!cmcf point to http_ctx.main_conf[ngx_http_core_module.ctx_index]!!!!
    cmcf = ctx->main_conf[ngx_http_core_module.ctx_index];
    cscfp = ngx_array_push(&cmcf->servers);//Means we save cscf in the http_ctx.main_conf[ngx_http_core_module.ctx_index]->servers!!!

    if (cscfp == NULL) {
        return NGX_CONF_ERROR;
    }
    *cscfp = cscf; //Save the server to the ngx_http_core_module's main_conf!!!

    /* parse inside server{} */
    pcf = *cf; //First we save the previous conf_ctx!!!
    cf->ctx = ctx;
    cf->cmd_type = NGX_HTTP_SRV_CONF;//the parse type is SRV_CONF

    rv = ngx_conf_parse(cf, NULL);//Here,cf->ctx still is ngx_http_conf_ctx_t!!! 

    *cf = pcf;//After parse we recover to previous conf_ctx!we will return to ngx_http_block!
    if (rv == NGX_CONF_OK && !cscf->listen) {
        ...
        if (ngx_http_add_listen(cf, cscf, &lsopt) != NGX_OK) {//create the lsopt!!!
            return NGX_CONF_ERROR;
        }
    }
    return rv;
...
//recite it,the ctx created by ngx_http_core_location will save to the uplayer ngx_http_conf_ctx_t.loc_conf[ngx_http_core_moudle->ctx_index]
//Means ngx_http_core_location's ctx save to loc_conf[ngx_http_core_module->ctx_index].locations!!!!
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }

    pctx = cf->ctx; //Here,Means get the server's ngx_http_conf_ctx_t
    ctx->main_conf = pctx->main_conf;//The new ctx inherit the server's main_conf
    ctx->srv_conf = pctx->srv_conf;  //The new ctx inherit the server's srv_conf

    ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);//Create New loc_conf!!!
    if (ctx->loc_conf == NULL) {
        return NGX_CONF_ERROR;
    }

//---------------->Create all the http_module's locale conf_ctx in the new ctx!!!!
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->type != NGX_HTTP_MODULE) {
            continue;
        }
...
//
    ...
    clcf->loc_conf = ctx->loc_conf;//save the ctx->loc_conf array address to ngx_http_core_loc_conf_t.loc_conf!!
								   
    value = cf->args->elts;//point to the command!!!
/***Now start to parse the location name!!!!****/
//location command no more than three,
//location  ~    ^.*\.(jpeg|png|gif)$ {}
//Means to say location is used for compare to a URI! 
//Means we can use different command according to different URI!!
    if (cf->args->nelts == 3) {
        ...
        if (len == 1 && mod[0] == '=') {//exact_match means match the name accurate!!!

            clcf->name = *name;
            clcf->exact_match = 1;

        } else if (len == 2 && mod[0] == '^' && mod[1] == '~') {//Means  Use regex,distinguish the upper case and lower case!!!
...
//---------------------------------->pay much attention!!!----------------------------------->
//This pctx = cf->ctx; //so the pctx is the uplayer ctx,Means the ctx is server {} block conf_ctx !!!!
//ngx_http_core_server() will call the ngx_http_core_module.ctx.ngx_http_core_create_loc_conf(),create all the http_module's loc_conf_ctx in 
//Here,pclcf is ngx_http_core_loc_conf_t type!!!
//ngx_http_core_create_loc_conf(ngx_conf_t *cf) just create a clcf,doesn't bind a name!
    pclcf = pctx->loc_conf[ngx_http_core_module.ctx_index];//Here,Means point the  server {} block's loc_conf!!! 

    if (pclcf->name.len) {//Means to get the uplayer loc_conf!!!
        /* nested location */
#if 0
	...
	//nginx doesn't like to use nested loction command!!!
	//Here,just add the clcf to the server pclcf->locations queue!!!
    if (ngx_http_add_location(cf, &pclcf->locations, clcf) != NGX_OK)
	{//Here Means add the location into the previous server's loc_conf!!!
        return NGX_CONF_ERROR;
    }

    save = *cf;//first save the previous conf_ctx
    cf->ctx = ctx;
    cf->cmd_type = NGX_HTTP_LOC_CONF;
//---------------->Here,very important,Why we will have nested locations????
//Because the current ctx is loc_conf_ctx!This ctx.loc_conf[ngx_http_core_module.ctx_index]->loc_conf stored is current ctx!!!
//This clcf has beed initilized and belong to a pointed location!!! The critical step are as follow!!!
//   clcf = ctx->loc_conf[ngx_http_core_module.ctx_index];
//   clcf->loc_conf = ctx->loc_conf;//This

    rv = ngx_conf_parse(cf, NULL);

    *cf = save;//Now we recover to previous conf_ctx! after parse we return to ngx_http_core_server;
...
ngx_http_core_regex_location(ngx_conf_t *cf, ngx_http_core_loc_conf_t *clcf, ngx_str_t *regex, ngx_uint_t caseless)
{
    ...
    clcf->regex = ngx_http_regex_compile(cf, &rc);//Means to compile a regex struct 
    if (clcf->regex == NULL) {
        return NGX_ERROR;
    }
...
/*See from follow macro we know that parent conf value have top priority when the server conf value is UNSET state!!!
